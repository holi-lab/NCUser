...
Rules
1. A turn is defined as a single utterance starting with 'User:' for the user and 'Agent:' for the tool agent. You must count anger and failure notifications across all turns. Even if anger or failure notification appears multiple times within the same turn, it is counted as 1.
...
anger_fail_function_spec = {
  ...
  "parameters": {
    ...
    "properties": {
      ...
      "anger_count": {
        "type": "integer",
        "description": "The number of user turns (User:) that express anger."
      },
      "failure_notify_count": {
        "type": "integer",
        "description": "The number of agent turns (Agent:) that contain a failure notification (excluding turns with apology only)."
      }
    },
...
def process_file(file_info):
    ...
    if "anger_fail.json" in os.listdir(f"experiment_result/ours/{model}/emotional_acts/{file}"):
        with open(f"experiment_result/ours/{model}/emotional_acts/{file}/anger_fail.json",'r') as f:
            anger_fail = json.load(f)
        if "reasoning" in anger_fail and "anger_count" in anger_fail and "failure_notify_count" in anger_fail:
            return

    with open(f"experiment_result/ours/{model}/emotional_acts/{file}/dialogue_history.txt",'r') as f:
        dial_hist = f.read()
    ...
    with open(f"experiment_result/ours/{model}/emotional_acts/{file}/anger_fail.json",'w') as f:
...
for model in model_name:
    for file in os.listdir(f"experiment_result/ours/{model}/emotional_acts"):
